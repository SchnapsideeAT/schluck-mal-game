...
const Game = () => {
  ...
  return <div className="min-h-screen flex flex-col p-6 relative mx-0 my-0">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <Button onClick={() => navigate("/")} variant="ghost" size="icon" className="hover:bg-muted/50">
          <Home className="w-5 h-5" />
        </Button>
        
        {/* Current Player Display */}
        {players.length > 0 && currentIndex >= 0 ? <div className="inline-flex items-center gap-2 bg-primary/10 border border-primary/30 rounded-full px-4 py-2">
        ...
        <div className="flex gap-2">
          <Button onClick={showStatistics} variant="ghost" size="icon" className="hover:bg-muted/50">
            <Trophy className="w-5 h-5" />
          </Button>
          <Button onClick={handleRestart} variant="ghost" size="icon" className="hover:bg-muted/50">
      ...
      <div className="flex-1 flex items-center justify-center">
        {currentIndex === -1 ? <div className="text-center space-y-6 slide-up cursor-pointer" onClick={() => drawCard()} onTouchStart={() => drawCard()}>
          ...
          </div> : showCard && currentCard ? <div className="w-full mx-auto relative">
            {/* Next card (behind) */}
            {currentIndex < deck.length - 1 && <div className="absolute inset-0 z-0">
                <GameCard card={deck[currentIndex + 1]} swipeDistance={0} swipeDirection={null} showGlow={false} />
              </div>}
            
            {/* Current card (on top) */}
            <div className="relative z-10">
              <GameCard card={currentCard} swipeDistance={swipeState.swipeDistance} swipeDirection={swipeState.swipeDirection} {...swipeHandlers} />
            </div>
          </div> : null}
      </div>
